...
for table_number,table in enumerate(soup.find_all('table',"wikitable plainrowheaders collapsible")):
    ...
    for row in table.find_all("tr"):
        ...
        if flag and len(cells) == 7:
            ...
            # Date
            date = cells[0].find('span', class_='nowrap').get_text(strip=True)
            launch_dict['Date'] = date

            # Mission
            mission = cells[1].get_text(strip=True)
            launch_dict['Mission'] = mission

            # Rocket
            rocket = cells[2].get_text(strip=True)
            launch_dict['Rocket'] = rocket
            ...
            # Launch outcome
            launch_outcome = cells[6].get_text(strip=True)
            launch_dict['Launch outcome'] = launch_outcome if launch_outcome else None

            # Append the dictionary to the launch_list
